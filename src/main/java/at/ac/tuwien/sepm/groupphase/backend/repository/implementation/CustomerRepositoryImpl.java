...
public class CustomerRepositoryImpl implements CustomerRepositoryCustom {
    ...
    public Page<Customer> findCustomersFiltered(Long id, String name, String firstName, String email, LocalDate birthday, Pageable pageable) {
        ...
        if(name != null){
            predicates.add(cBuilder.like(cBuilder.lower(customerRoot.get(Customer_.lastName)), "%" + name.toLowerCase() + "%"));
        }
        if(firstName != null){
            predicates.add(cBuilder.like(cBuilder.lower(customerRoot.get(Customer_.firstName)), "%" + firstName.toLowerCase() + "%"));
        }
        if(email != null){
            predicates.add(cBuilder.like(cBuilder.lower(customerRoot.get(Customer_.username)), "%" + email.toLowerCase() + "%"));
        }
        if(birthday != null){
            predicates.add(cBuilder.equal(customerRoot.get(Customer_.birthday), birthday));
        }
        //order
        criteriaQuery.orderBy(cBuilder.asc(customerRoot.get(Customer_.id))).orderBy(cBuilder.asc(customerRoot.get(Customer_.lastName))).orderBy(cBuilder.asc(customerRoot.get(Customer_.firstName)));
        //excecute Query
